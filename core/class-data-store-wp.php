 ...
 * Your own data store doesn't need to use Data_Store_WP -- you can write
...
class Data_Store_WP {
	 ...
	 */
	protected function get_term_ids( $object, string $taxonomy ): array {
	...
	public function add_meta( Data &$object, object $meta ): int {
		return add_metadata(
			...
			$object->get_id(),
			wp_slash( $meta->key ),
			is_string( $meta->value ) ? wp_slash( $meta->value ) : $meta->value,
			false
		);
	}
	 ...
	 * @return bool
	 */
	protected function exclude_internal_meta_keys( $meta ): bool {
	 ...
	 */
	protected function get_props_to_update( Data $object, array $meta_key_to_props, $meta_type = 'post' ): array {
	 ...
	 * @return array
	 */
	protected function get_data_for_lookup_table( int $id, string $table ): array {
		return array();
	}
